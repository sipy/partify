...
class PHPWebSocket
{
	...
	const WS_STATUS_NORMAL_CLOSE =                      WS_STATUS_NORMAL_CLOSURE;
	const WS_STATUS_GONE_AWAY =                         WS_STATUS_GONE_AWAY;
	const WS_STATUS_PROTOCOL_ERROR =                    WS_STATUS_PROTOCOL_ERROR;
	const WS_STATUS_UNSUPPORTED_MESSAGE_TYPE =          WS_STATUS_UNSUPPORTED_MESSAGE_TYPE;
	const WS_STATUS_MESSAGE_TOO_BIG =                   WS_STATUS_MESSAGE_TOO_BIG;

	const WS_STATUS_TIMEOUT =                           WS_STATUS_TIMEOUT;
	...
	public $wsVcs		    = array();
	public $wsVcsCount	    = 0;
	public $wsVcsClientCount    = array();
	public $wsVcsClientIPCount  = array();
	public $wsAllowedOrigins    = array();
	...
	*/
	...
	function wsStartServer($host, $port) {
                ...
                $this->wsAllowedOrigins = $ALLOWED_ORIGINS;
	...
	function wsProcessClientHandshake($clientID, &$buffer) {
		...
		for ($i=1; $i<$headersCount; $i++) {
			...
			$headersKeyed[trim($parts[0])] = trim($parts[1].(trim($parts[0])=="Origin"&&isset($parts[2])?":".$parts[2]:""));
		}
                
                // check if Origin is allowed
                if (!isset($headersKeyed['Origin'])||
                    (isset($headersKeyed['Origin']) && !in_array($headersKeyed['Origin'], $this->wsAllowedOrigins))) return false;
		...
		$headers = array(
			...
			'Sec-WebSocket-Accept: '.$hash,
                        'Access-Control-Allow-Origin: '.$headersKeyed['Origin'],
