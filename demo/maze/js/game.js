...
function game_closeWinWindowListener() {
    ...
    $(document).on("click touchstart", "#winWindow", function() {
        ...
        if(partify) partify.triggerVcEvent(closeWinWindow);
    });
}
...
function game_closeHelpWindowListener() {
    ...
    $(document).on("click touchstart", "#helpWindow", function() {
        ...
        if(partify) partify.triggerVcEvent(closeHelpWindow);
    });
}
...
function game_getGameState() {
    game_clearGame();
    if(partify){
        gamePlayers = partify.getVcCompState("maze").gamePlayers;
        helpArrows = partify.getVcCompState("maze").helpArrows;
        helpTouch = partify.getVcCompState("maze").helpTouch;
        if (gamePlayers) {
            $.each(gamePlayers, function(index, playerObj) {
                if (playerObj && playerObj != -1) {
...
function game_setGameState() {
    if(partify) partify.setVcCompState("maze", {"helpArrows": helpArrows, "helpTouch": helpTouch, "gamePlayers": gamePlayers});
}
